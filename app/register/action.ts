  ...
  try {
    ...
    try {
      ...
      if (authError || !authData?.user) {
        ...
        return { success: false, error: authError?.message || "Authentication registration failed" }
      }
      ...
      if (errorMessage.includes('ENOTFOUND') || errorMessage.includes('fetch')) {
        return { 
          success: false, 
          error: "Unable to reach the registration server. Please check your internet connection and try again." 
        }
      }
      return { success: false, error: errorMessage }
    }
  } catch (error) {
    console.error('Sign up outer error:', error)
